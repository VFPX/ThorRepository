...
IF PCOUNT() = 1 ;
	...
	WITH lxParam1
		...
		* Required
		.Prompt		   = 'Convert files with changed internal timestamps'
		
		* Optional
		TEXT TO .Description NOSHOW && a description for the tool
Optimized conversion of all VFP binary files in the ActiveProject to FoxBin2Prg text format.

All files that have been changed using either the Designer are re-converted, as this tool looks at the timestamps in each row to determined whether the file has been changed..

However, there is *DANGER* if you hack a file (by HackCX or manually) as the text file may not be automatically created.  There are a number of ways around this:
    - Use tool "Convert most recently changed binary files"
    - Open the file in the Designer, make a slight modification to it, and save it.
...
TRY
	...
	* If no project was selected, abort.
	...
	ExecScript(_Screen.cThorDispatcher, 'Thor_Proc_FoxBin2PRGChangedFilesInProject.PRG', m.lcProjectName)
